...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'inventory' | 'stats' | 'analytics' | 'history' | 'settings'>('inventory');
  ...
  const [showNotificationsBanner, setShowNotificationsBanner] = useState(notifications.length > 0);
    ...
    <div className="relative min-h-screen bg-black text-white font-sans selection:bg-emerald-500/30">
      ...
      <header className="px-4 sm:px-6 pt-6 pb-4 sticky top-0 bg-black/80 backdrop-blur-xl z-50 border-b border-white/5">
        <div className="max-w-5xl mx-auto">
            ...
            <div className="flex items-center gap-2">
              {notifications.length > 0 && (
                <button 
                  ...
                  className="p-2.5 bg-blue-500/10 rounded-xl border border-blue-500/20 text-blue-400 hover:bg-blue-500/20 hover:border-blue-500/30 transition-all cursor-pointer relative"
                >
                  <Bell className="w-4 h-4" />
                  <span className="absolute -top-1 -right-1 w-4 h-4 bg-blue-500 rounded-full text-white text-[9px] font-black flex items-center justify-center">
          ...
          {notifications.length > 0 && showNotificationsBanner && (
            <motion.div 
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mt-4 bg-blue-500/10 border border-blue-500/30 rounded-xl p-3 flex items-start gap-3"
            >
              <Bell className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
              <div className="flex-1 min-w-0">
                <p className="text-xs text-blue-300 font-semibold mb-1">{notifications.length} Alert(s)</p>
                <div className="space-y-1">
                  {notifications.slice(0, 2).map((n, i) => (
                    <p key={i} className="text-xs text-blue-200/70 truncate">{n}</p>
                  ))}
                </div>
              </div>
              <button 
                onClick={() => setShowNotificationsBanner(false)}
                className="text-xs text-blue-400 hover:text-blue-300 shrink-0 font-semibold"
              >
                ✕
              </button>
            </motion.div>
      ...
      <main className="relative z-10 px-3 sm:px-4 pb-28 max-w-5xl mx-auto">
        ...
        {activeTab === 'analytics' && (
          <Dashboard phones={phones} expenses={expenses} />
        )}
        {activeTab === 'settings' && (
